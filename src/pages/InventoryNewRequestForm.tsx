...
import {
  Loader2, Plus, Trash2, ArrowDownToLine, ArrowUpFromLine, Undo2, Redo2,
  ArrowRight,
} from "lucide-react";
import type { InventoryItem, Warehouse, PermitType } from "@/lib/finhub-types";
import { PERMIT_TYPE_LABELS_AR } from "@/lib/finhub-types";
...
const TYPE_META: Record<PermitType, { icon: typeof ArrowUpFromLine; usesCost: boolean; counterpartyHint: string }> = {
  issue: { icon: ArrowUpFromLine, usesCost: false, counterpartyHint: "حساب العميل / المصروف" },
  receive: { icon: ArrowDownToLine, usesCost: true, counterpartyHint: "حساب المورد / النقدية" },
  sales_return: { icon: Undo2, usesCost: false, counterpartyHint: "حساب العميل المرجع منه" },
  purchase_return: { icon: Redo2, usesCost: true, counterpartyHint: "حساب المورد المرجع له" },
};

const VALID_TYPES: PermitType[] = ["issue", "receive", "sales_return", "purchase_return"];

const InventoryNewRequestForm = () => {
  ...
  const permitType: PermitType = (VALID_TYPES.includes(type as PermitType) ? type : "issue") as PermitType;
  const meta = TYPE_META[permitType];
  ...
  const onPickItem = (uid: string, itemId: string) => {
    ...
    updateLine(uid, {
      item_id: itemId,
      unit_price: item ? String(meta.usesCost ? item.cost_price : item.sale_price) : "0",
    });
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto" dir="rtl">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/inventory/new-request")}>
        ...
        <div>
          <h1 className="font-display text-2xl font-bold">{PERMIT_TYPE_LABELS_AR[permitType]}</h1>
          <p className="text-sm text-muted-foreground">املأ بيانات الطلب وأرسله للمراجعة.</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <SearchableSelect
                ...
                placeholder={meta.counterpartyHint}
              />
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[15%]">سعر الوحدة (مبدئي)</TableHead>
          ...
          <div className="flex items-center justify-between border-t p-4 bg-muted/30">
            <span className="text-sm text-muted-foreground">الإجمالي المبدئي</span>
      ...
      <div className="flex items-center justify-end gap-3">
        <Button variant="outline" onClick={() => navigate("/inventory/new-request")}>إلغاء</Button>
...
export default InventoryNewRequestForm;
